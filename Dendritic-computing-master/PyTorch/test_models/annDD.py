...
class ddNet(nn.Module):
    ...
    def __init__(self,channels):
        ...
        # xw+b
        self.dd = nn.Linear(channels, channels, bias=False)
...
class DendriteandDD(nn.Module):

    def __init__(self,channels,
                 nonlinearity=0.1,
                 # nonlinearity=0.25,
                 ):
        super(DendriteandDD, self).__init__()
        self.a = torch.tensor(50 * nonlinearity / (1 - nonlinearity))
        self.ddNET=ddNet(channels)
        # self.a = torch.tensor(6.18)
    ...
    def forward(self, x):
        ...
        if self.a <= 0:
            ...
            out = self.ddNET((torch.exp(self.a * x) - 1) / (torch.exp(self.a) - 1))
        xmax = out.abs().max(dim=1)[0]
        out = out / xmax.unsqueeze(1)
        return out
...
class Layer(nn.Module):
    ...
    def __init__(self,
        ...
        self.soma = Soma()
        self.dendrite = DendriteandDD(channels=out_channel)
